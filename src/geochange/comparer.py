def colonnes_ajoutees(gdf_old, gdf_new):
    ...
    if len(colonnes_ajoutees) == 0:
        print("Aucune colonne n'a été ajoutée.")
    else:
...
def entites_ajoutees(gdf_old, gdf_new, colonne_id):
    ids_old = set(gdf_old[colonne_id])
    ids_new = set(gdf_new[colonne_id])

    ids_ajoutes = ids_new - ids_old
    return ids_ajoutes
    
def entites_supprimees(gdf_old, gdf_new, colonne_id):
    ids_old = set(gdf_old[colonne_id])
    ids_new = set(gdf_new[colonne_id])

    ids_supprimes = ids_old - ids_new

    return ids_supprimes